let players = ["rohit", "gill", "virat", "iyer", "rahul", "jadeja", "bumeah"];
...
employees.map((emp) => {
  ...
  div.innerHTML = `<h2>Ename is ${emp.name}</h2>
  <h2>Eid is ${emp.id}</h2> <h2>age is ${emp.age}</h2><h2>department is ${emp.department}</h2><h2>Salary is ${emp.salary}</h2>`;
  main.append(div);
});
